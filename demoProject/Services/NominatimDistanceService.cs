...
namespace demoProject.Services
{
    ...
    private readonly ILogger<NominatimDistanceService> _logger;
        private static readonly Dictionary<string, (double lat, double lon, DateTime cached)> _geocodeCache = new();
  ...
  public NominatimDistanceService(HttpClient httpClient, ILogger<NominatimDistanceService> logger)
        {
...
// Set required headers for Nominatim API
            ...
            _httpClient.DefaultRequestHeaders.Add("User-Agent", "LogisticTracker/1.0 (contact@logistictracker.com)");
...
if (_geocodeCache.ContainsKey(cacheKey))
                ...
                if (DateTime.UtcNow - cached.cached < _cacheExpiry)
 {
  ...
  if (double.TryParse(result.lat, out var lat) && double.TryParse(result.lon, out var lon))
       ...
       _logger.LogDebug("Successfully geocoded address: {Address} -> ({Lat}, {Lon})", 
                address, lat, lon);
       return (lat, lon);
          }
  }
 ...
 _logger.LogError(ex, "Timeout while geocoding address: {Address}", address);
  ...
  {
         _logger.LogError(ex, "JSON parsing error while geocoding address: {Address}", address);
            }
     catch (Exception ex)
          {
                _logger.LogError(ex, "Unexpected error while geocoding address: {Address}", address);
            }
  ...
  return (0, 0);
      ...
      return 25; // Default fallback distance
            ...
            try
 {
         _logger.LogDebug("Calculating distance between: {Origin} and {Destination}", 
 originAddress, destinationAddress);
   ...
   var destCoords = await GeocodeAddressAsync(destinationAddress);
     ...
     if (originCoords.latitude == 0 || destCoords.latitude == 0)
                {
       _logger.LogWarning("Failed to geocode one or both addresses. Origin: {Origin}, Destination: {Destination}", 
             originAddress, destinationAddress);
   ...
   destCoords.latitude, destCoords.longitude);
          ...
          _logger.LogDebug("Calculated distance: {Distance} km between {Origin} and {Destination}", 
      distance, originAddress, destinationAddress);
