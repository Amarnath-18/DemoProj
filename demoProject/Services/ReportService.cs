...
namespace demoProject.Services
{
    ...
    {
        Task<string> GenerateShipmentReportAsync(DateTime startDate, DateTime endDate, ReportType reportType, Guid userId);
                        ...
                        .Column(x =>
                        {
                            ...
                            x.Item().Row(row =>
                            {
                                ...
                                row.RelativeItem().Column(col =>
                                {
                                    ...
                                    col.Item().Text(shipments.Count(s => s.Status == ShipmentStatus.Delivered).ToString());
                                });

                                row.RelativeItem().Column(col =>
                                {
                                    col.Item().Text("In Transit").SemiBold();
                                    col.Item().Text(shipments.Count(s => s.Status == ShipmentStatus.InTransit).ToString());
                                });

                                row.RelativeItem().Column(col =>
                                {
                                    col.Item().Text("Pending").SemiBold();
                                    col.Item().Text(shipments.Count(s => s.Status == ShipmentStatus.Created).ToString());
                                });
                            });
                            ...
                            x.Item().Table(table =>
                            {
                                ...
                                {
                                    table.Cell().Element(CellStyle).Text(shipment.TrackingNumber);
                                    table.Cell().Element(CellStyle).Text(shipment.Sender.FullName);
                                    table.Cell().Element(CellStyle).Text(shipment.ReceiverName);
                                    table.Cell().Element(CellStyle).Text(shipment.DestinationAddress);
                                    table.Cell().Element(CellStyle).Text(shipment.Status.ToString());
